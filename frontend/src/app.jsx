...
// Source of truth: edit this file, then run `npm run build` in frontend/.
...
// ═══════════════════════════════════════════════════════════════════
function TopBar({ view, runningJob, awaitingReviewCount, onJumpToProcessing, onJumpToReview }) {
  const TITLES = {
...
// ═══════════════════════════════════════════════════════════════════
function ProcessingView({ runningJob, sphereOn, onJobCancelled, onSwitchToHome }) {
...
function ProcessingEmpty({ onSwitchToHome }) {
  return (
...
function StageRow({ stage, idx, state, stepDetail, isLast }) {
...
// ═══════════════════════════════════════════════════════════════════
function HistoryView({ jobs, onJumpProcessing, onJumpReview, onJumpResult, onRefresh }) {
...
function JobRow({ job, onJumpProcessing, onJumpReview, onJumpResult, onRefresh }) {
...
function RedubModal({ job, onClose, onSubmitted }) {
...
function RowActions({ job, busy, onView, onWatch, onReview, onResume, onCancel, onRemove, onRedub }) {
...
// ═══════════════════════════════════════════════════════════════════
function ResultView({ jobs, selectedJobId, voicePresets, onPickJob, onSwitchToHome, onSwitchToHistory }) {
...
function ResultViewInner({ job, jobs, voicePresets, onPickJob, onSwitchToHistory }) {
...
// ═══════════════════════════════════════════════════════════════════
function EditorView({ jobs, selectedJobId, onPickJob, onSwitchToHome }) {
  const completedJobs = useMemo(
    () => jobs.filter(j => j.status === 'complete').sort((a, b) => (b.completed_at || b.created || 0) - (a.completed_at || a.created || 0)),
    [jobs]
  );
...
function ExpandPanel({ open, onToggle, title, subtitle, icon, children }) {
  return (
    <div style={{
...
// ── Burn subs panel ──────────────────────────────────────────────────
function SubsPanel({ job, onBurned }) {
  const STYLES = [
...
// ── Lip-sync panel ──────────────────────────────────────────────────
function LipSyncPanel({ job, onSynced }) {
...
function RegenPanel({ job, voicePresets, onSubmitted }) {
...
function ReviewViewInner({ job, reviewJobs, voicePresets, onContinued, onCancel, onPickJob }) {
...
function SpeakerCard({ sp, jobId, selected, segCount, onClick, onReplaced }) {
...
function SegmentRow({ seg, selected, onSelect, editedValue, onEdit, onRevert }) {
...
function QuickTestBatchCard({ batch }) {
...
function ShowcaseBatchCard({ batch }) {
...
function BatchCard({ batch, onJumpProcessing, onJumpResult, onRefresh }) {
...
function BatchJobRow({ job, onJumpProcessing, onJumpResult }) {
...
// ── Models tab ──────────────────────────────────────────────────────
function SysModels({ system, onRefresh }) {
...
function ModelRow({ model, installed, last, onChanged }) {
  const [busy, setBusy] = useState(false);
...
function AddonCard({ name, detail, installed, size, steps, onRefresh, repoDir }) {
...
// ═══════════════════════════════════════════════════════════════════
function ComingSoonView({ title, hint, sessionLabel }) {
  return (
...
function VoiceCard({ preset, onEdit, onDeleted }) {
...
function VoiceUploadModal({ onClose, onUploaded }) {
...
function VoiceEditModal({ preset, onClose, onSaved }) {
...
function GlossaryDomainCard({ domain, onChange, onRemove }) {
